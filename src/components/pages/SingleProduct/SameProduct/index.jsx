  ...
  return (
    <Box>
      <Box className="SameProduct">
        <ArrowBackIosNewIcon className="pagination-icon pagination-icon-samePage" onClick={ backBtn} />
        {paginate(eyesGlassess, 5, +page)?.map((item) => (
          <MyCard
            item={item}
            title={item.title}
            price={item.price}
        ...
        <ArrowForwardIosIcon className="pagination-icon pagination-icon-samePage" onClick={forwardBtn} />
      </Box>
    </Box>
  );
}
